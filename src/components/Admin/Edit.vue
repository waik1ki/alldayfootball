...
export default {
    ...
    watch:{
        $route(){
            ...
            axios.post(`${this.thisUrl}api/board/findone`,{
                seq:n
            })
            .then((res)=>{
    ...
    computed:{
        radioGroup(){
            switch(this.select){
                ...
                case this.bNumItems[8] : return 8;
                case this.bNumItems[9] : return 9;
                default: return 0;
            }
        },
    },
    methods:{
        ...
        getConfig(){
            axios.post(`${this.thisUrl}api/config/findone`,{
        ...
        findSelect(n){
            switch(n){
                ...
                case 8 :  this.select =this.bNumItems[8]; break;
                case 9 :  this.select =this.bNumItems[9]; break;
        ...
        clickSubmit(){
            ...
            this.pretext = this.editor.value.replace(/(<([^>]+)>|&nbsp;|@([^>]+)})/ig,"").slice(0,150)+('...');
            axios.put(`${this.thisUrl}api/board/edit`,{
                id: this.id,
                thumb: this.thumb,
                title: this.title,
        ...
        clickdelete(){
            axios.post(`${this.thisUrl}api/board/delete`,{
                id: this.id
            })
            .then(()=>{
                window.location.href = "/admin/article"
