    ...
    >
      ...
      <v-col style="border-right:1px solid rgba(0,0,0,.2);" cols="8">
        <v-card height="150px" rounded="0">
          <v-img height="100%" :src="i.image"></v-img>
        </v-card>
      </v-col>
      <v-col
        class="d-flex align-center justify-center"
    ...
    <v-row>
      <v-col class="d-flex justify-end" cols="12">
        <v-btn @click="dialog = true" color="#509F3F" class="mr-10 my-5"
          ...
          <p class="subText" style="color:white;">배너 등록</p></v-btn
        >
      </v-col>
    </v-row>
    <!-- 배너등록 -->
      ...
      <v-container style="background-color: #FBFBFB;" class="white">
        ...
        <v-row class="d-flex justify-center">
          <v-col class="d-flex justify-space-around" cols="12">
            <v-btn @click="uploadImage" color="#509F3F"
  ...
  methods: {
    ...
    async uploadImage() {
      ...
      const res = await axios.post(
        `${this.thisUrl}api/storage/adupload`,
        formData,
        {
          headers: {
      ...
      await axios
        .post(`${this.thisUrl}api/ad/upload`, {
          name: this.adName,
          image: this.adUrl,
        })
        .then(res => {
    ...
    getAdList() {
      axios.get(`${this.thisUrl}api/ad/find`).then(res => {
    ...
    remove(data) {
      axios
        .post(`${this.thisUrl}api/ad/delete`, {
          id: data,
        })
        .then(res => {
          if (res.data === 'not_logged') {
