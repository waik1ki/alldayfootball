  ...
  <div class="Accout">
    ...
    <v-row>
      <v-col class="d-flex justify-end" cols="12">
        <v-btn @click="dialog = true" color="#509F3F" class="mr-10 my-5"
      ...
      <v-container style="background-color: #FBFBFB;" class="white">
        <v-row>
          <v-col class="black d-flex align-center" cols="12" lg="12">
            ...
            <v-btn @click="closeDialog(0)" class="ml-auto" icon
      ...
      <v-container style="background-color: #FBFBFB;">
        <v-row>
          <v-col class="black d-flex align-center" cols="12" lg="12">
            ...
            <v-btn @click="closeDialog(1)" class="ml-auto" icon
              ><v-icon color="white">mdi-close</v-icon></v-btn
            >
          </v-col>
        </v-row>
        ...
        <v-row v-if="!$vuetify.mdAndDown" class="d-flex justify-center">
          <v-col class="d-flex justify-space-around" cols="12" lg="12">
            <v-btn @click="edit" color="#509F3F"
              ...
              <p class="subText" style="color:white;">등록하기</p></v-btn
            >
            <v-btn @click="remove" color="#737373"
        ...
        <v-row v-if="$vuetify.mdAndDown" class="d-flex justify-center">
          <v-col class="d-flex justify-space-around" cols="12" lg="12">
            ...
            >
            <v-btn x-small @click="remove" color="#737373"
              ><v-icon color="white">mdi-trash-can-outline</v-icon>
...
export default {
  data() {
    return {
      userList: [{ name: '', id: '', superAdmin: true, Admin: true }],
      dialog: false,
      dialog2: false,
      regId: '',
      regName: '',
  ...
  methods: {
    ...
    getUserList() {
      axios.get(`${this.thisUrl}api/auth/find`).then(res => {
    ...
    async register() {
      ...
      if (this.regFile != '') {
        ...
        const res = await axios.post(
          `${this.thisUrl}api/storage/profileupload`,
          formData,
          {
            headers: {
      ...
      await axios
        .post(`${this.thisUrl}api/auth/register`, {
          id: this.regId,
          name: this.regName,
          email: this.regEmail,
          photo: photoURL,
        ...
        .then(res => {
          ...
          if (res.data === 'registered') {
            ...
            this.regPassword = '';
            this.regRadio = 0;
            (this.regFile = ''), (this.dialog = false);
            return;
          }
    ...
    async edit() {
      ...
      await axios
        .put(`${this.thisUrl}api/auth/edit`, {
          id: this.editId,
          name: this.editName,
    ...
    remove() {
      axios
        .post(`${this.thisUrl}api/auth/delete`, {
          id: this.editId,
        })
        .then(res => {
          if (res.data === 'not_logged') {
